...
def extract_profile_data(profile_file: Path) -> Dict[str, Any]:
    ...
    profile_info = {
        ...
        'max_wcet': max(wcets),
        'min_wcet_period': round(min(utilizations), 1), # round to 0.x digit.
        'mean_wcet_period': round(sum(utilizations) / len(utilizations), 1), # round to 0.x digit.
        'max_wcet_period': round(max(utilizations), 1), # round to 0.x digit.
...
def main():
    ...
    #     'Task Count', 'Error Percentage (%)',
    #     show_plot=True, save_plot=True, output_dir=total_analysis_dir,
    #     legend_loc='upper right', fontsize=12,
    ...
    draw_multi_category_scatter_plot(
        total_df, 'max_period', 'error_percentage', 
        ['task_count', 'max_wcet_period'],
        'Max Period vs Error Percentage by Max Period and Max Utilization',
        r'Period $T$ (ms)', r'Error percentage $\Delta/l(\overleftarrow{ac})$ (%)',
        show_plot=True, save_plot=True, output_dir=total_analysis_dir,
