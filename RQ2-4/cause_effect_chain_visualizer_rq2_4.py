...
class GanttChartConfig:
    ...
    x_min: Optional[float] = None  # Minimum x-axis value (None = auto)
    x_max: Optional[float] = None  # Maximum x-axis value (None = auto)
    x_tick_interval: Optional[float] = None  # X-axis tick interval (None = auto)
    x_tick_labels: Optional[List[tuple]] = None  # Custom x-tick labels [(value, label), ...]
    # Display control
    show_title: bool = True  # Whether to show the chart title
    font_size: int = 12  # Global font size for all text elements
...
class CauseEffectChainVisualizer:
    ...
    def _setup_chart_axes(self, ax: plt.Axes, task_names: List[str], max_time: int):
        ...
        # Set x-axis (time) with custom range control
        x_min = self.config.x_min if self.config.x_min is not None else 0
        x_max = self.config.x_max if self.config.x_max is not None else max_time
        ...
        # Set custom x-axis ticks if specified
        if self.config.x_tick_interval is not None:
            x_ticks = np.arange(x_min, x_max + self.config.x_tick_interval, self.config.x_tick_interval)
            ax.set_xticks(x_ticks)
        elif self.config.x_tick_labels is not None:
            x_values = [tick[0] for tick in self.config.x_tick_labels]
            x_labels = [tick[1] for tick in self.config.x_tick_labels]
    ...
    def _plot_execution_blocks(self, task_result: Any, ax: plt.Axes, y_position: float, task_name: str, max_time: int):
        ...
        for read_event, write_event in zip(read_events, write_events):
            ...
            # Only plot if within x-range
            if (start_time >= x_min and start_time <= x_max) or (end_time >= x_min and end_time <= x_max):
    ...
    def _plot_release_markers(self, task_result: Any, ax: plt.Axes, y_position: float, task_name: str):
        ...
        # Get x-range limits
        x_min = self.config.x_min if self.config.x_min is not None else 0
        x_max = self.config.x_max if self.config.x_max is not None else float('inf')
    ...
    def _customize_chart(self, fig: plt.Figure, ax: plt.Axes, max_time: int, num_tasks: int):
        ...
        if self.config.show_title:
            ax.set_title(self.config.title, fontsize=self.config.font_size + 2, weight='bold', pad=20)
        ...
        ax.legend(handles=legend_elements, loc='lower left', bbox_to_anchor=(0.005, 0.16), fontsize=self.config.font_size)
    ...
    def _add_cause_effect_annotations(self, ax: plt.Axes, estimation_result: Any, 
        ...
        for chain in estimation_result.oracle_chains:
            ...
            if hasattr(chain, '_display_length'):
                oracle_chain_length = chain._display_length
        ...
        if hasattr(estimation_result, 'estimated_chains') and estimation_result.estimated_chains:
            for chain in estimation_result.estimated_chains:
                ...
                if hasattr(chain, '_display_length'):
                    estimated_chain_length = chain._display_length
        
        # Display combined chain length information
        if oracle_chain_length is not None and estimated_chain_length is not None:
            ...
            error_percentage = (error / oracle_chain_length) * 100 if oracle_chain_length > 0 else 0
            
            # Create combined text box
            text_content = [
                f"Oracle Chain Length: {oracle_chain_length:.1f}ms",
                f"Estimated Chain Length: {estimated_chain_length:.1f}ms",
                f"Estimation Error: {error:.1f}ms ({error_percentage:.1f}%)"
            ]
            
            # Display combined information - left-aligned at bottom
            ax.text(0.035, 0.03, 
                   '\n'.join(text_content), 
                   transform=ax.transAxes, fontsize=self.config.font_size, 
                   verticalalignment='bottom', horizontalalignment='left',
                   bbox=dict(boxstyle='round', facecolor='white', edgecolor='0.8', linewidth=1, alpha=0.9))
        elif oracle_chain_length is not None:
            # Only oracle chain available
            ax.text(0.035, 0.03, 
                   f"Oracle Chain Length: {oracle_chain_length:.1f}ms", 
                   transform=ax.transAxes, fontsize=self.config.font_size, 
                   verticalalignment='bottom', horizontalalignment='left',
                   bbox=dict(boxstyle='round', facecolor='white', edgecolor='0.8', linewidth=1, alpha=0.9))
        elif estimated_chain_length is not None:
            # Only estimated chain available
            ax.text(0.035, 0.03, 
                   f"Estimated Chain Length: {estimated_chain_length:.1f}ms", 
                   transform=ax.transAxes, fontsize=self.config.font_size, 
                   verticalalignment='bottom', horizontalalignment='left',
    ...
    def _visualize_single_chain(self, ax: plt.Axes, chain: Any, task_name_to_y: Dict[str, int], 
        ...
        if hasattr(chain, 'z_event') and chain.z_event is not None:
            ...
            if first_task_name in task_name_to_y:
                ...
                # Only draw if within x-range
                if (x_min <= z_time <= x_max) or (x_min <= first_task_read_time <= x_max):
        ...
        if hasattr(chain, 'z_prime_event') and chain.z_prime_event is not None:
            ...
            if last_task_name in task_name_to_y:
                ...
                # Only draw if within x-range
                if (x_min <= z_prime_time <= x_max) or (x_min <= last_task_write_time <= x_max):
        ...
        # Store chain length for later combined display
        if hasattr(chain, 'chain_length') and chain.chain_length is not None:
            # Store the chain length in the chain object for later use
            chain._display_length = chain.chain_length
        ...
        for i, task_pair in enumerate(chain.task_sequence):
            ...
            # Only draw if within x-range
            if (x_min <= read_time <= x_max) or (x_min <= write_time <= x_max):
                ...
                ax.plot(read_time, y_position, 'ro', markersize=8, alpha=0.8)
                ax.plot(write_time, y_position, 'ro', markersize=8, alpha=0.8)
        ...
        for i in range(len(chain.task_sequence) - 1):
            ...
            # Only draw if within x-range
            if (x_min <= start_x <= x_max) or (x_min <= end_x <= x_max):
